  ...
  try {
    ...
    const cliente = await prisma.cliente.create({
      data: {
        ...
        usava_dashboard: body.usava_dashboard || (body.recursos_ud || []).some((r: string) => r.toLowerCase().includes('relatórios') || r.toLowerCase().includes('dashboard')),
        usava_plugin: body.usava_plugin || (body.plugins_rastreio || []).some((p: string) => p.includes('UnicoDrop')) || (body.recursos_ud || []).some((r: string) => r.toLowerCase().includes('rastreio')),
        usava_whatsapp: body.usava_whatsapp || (body.recursos_ud || []).some((r: string) => r.toLowerCase().includes('whatsapp')),
